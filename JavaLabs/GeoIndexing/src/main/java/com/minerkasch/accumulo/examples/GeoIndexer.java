...
public class GeoIndexer extends Configured implements Tool {
  ...
  public int run(String[] args) throws Exception {
    ...
    AccumuloInputFormat.setConnectorInfo(
        job, Constants.USER_NAME, new PasswordToken(Constants.USER_PASS));
    ...
    AccumuloOutputFormat.setDefaultTableName(job, Constants.TWEET_LOCATION_INDEX_TABLE);
  ...
  public static class GeoMapper extends Mapper<Key, Value, Text, Text> {
    ...
    public void map(Key key, Value value, Context context)
      ...
      if (key.getColumnFamily().equals(TWEET_CF) && key.getColumnQualifier().equals(LAT_CQ)) {
      ...
      if (key.getColumnFamily().equals(TWEET_CF) && key.getColumnQualifier().equals(LON_CQ)) {
      ...
      if (latitude != null && longitude != null) {
        ...
        if (latID.equals(lonID)) {
          ...
          encodedLocation = GeoUtil.encodeLocation(latitude, longitude);
