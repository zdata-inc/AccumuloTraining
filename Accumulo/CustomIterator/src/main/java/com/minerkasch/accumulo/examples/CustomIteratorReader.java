...
public class CustomIteratorReader extends Configured implements Tool {
  ...
  public static void main(String[] args) throws Exception {
    System.exit(ToolRunner.run(new Configuration(), new CustomIteratorReader(), args));
  }
  ...
  public int run(String[] arg0) throws Exception {
    ...
    if (!connector.tableOperations().exists(Constants.TWITTER_TABLE)) {
      System.out.format("Error: Table %s does not exist", Constants.TWITTER_TABLE);
      return 1;
    }
    ...
    Scanner scanner =
        new ClientSideIteratorScanner(
            connector.createScanner(Constants.TWITTER_TABLE, new Authorizations()));

    // Configure the iterator and add it to the scanner
    IteratorSetting iterConfig =
        new IteratorSetting(21, "rowEnumeratingIterator", RowEnumeratingIterator.class);
