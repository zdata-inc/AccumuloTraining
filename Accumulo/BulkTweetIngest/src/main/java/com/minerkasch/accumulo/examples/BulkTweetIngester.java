...
public class BulkTweetIngester extends Configured implements Tool {
  ...
  public static void main(String[] args) throws Exception {
    System.exit(ToolRunner.run(new Configuration(true), new BulkTweetIngester(), args));
  }
  ...
  public int run(String[] args) throws Exception {
    ...
    if (args.length != 2) {
      System.err.println("Usage: accumulo-bulk-ingest-example.jar <input_dir> <output_dir>");
      return 1;
    }
    ...
    Connector connector =
        instance.getConnector(Constants.USER_NAME, new PasswordToken(Constants.USER_PASS));
    ...
    SortedSet<Text> splitPoints =
        FileSampling.getSplits(conf, inputDir, samplerWorkingDir, .01, 10);
    ...
    connector.tableOperations().addSplits(Constants.TWITTER_TABLE, splitPoints);
    ...
    if (code == 0) {
      ...
      fs.setPermission(rfileOutput, new FsPermission(FsAction.ALL, FsAction.ALL, FsAction.ALL));

      // Create a failures directory in HDFS and set open permissions
      fs.mkdirs(failures);
      fs.setPermission(failures, new FsPermission(FsAction.ALL, FsAction.ALL, FsAction.ALL));

      // Import the raw RFiles into Accumulo
      connector
          .tableOperations()
          .importDirectory(
              Constants.TWITTER_TABLE, rfileOutput.toString(), failures.toString(), false);
    }
    return code;
  }

  public static class TweetMapper extends Mapper<LongWritable, Text, Key, Value> {
    ...
    public void map(LongWritable keyIn, Text valueIn, Context context)
      ...
      try {
        ...
        Object o = TwitterObjectFactory.createObject(valueIn.toString());
        Status tweet = null;
        ...
        if (o instanceof Status) {
          ...
          if (tweet.getUser().getLang().equals("en")) {
            ...
            if (tweet.getGeoLocation() != null) {
              ...
              outvalue.set(Double.toString(tweet.getGeoLocation().getLatitude()).getBytes());
              context.write(outkey, outvalue);

              // (tweetId, "tweet", "lon", geoLoc.lon)
              outkey.set(new Key(tweetId, "tweet", "lon"));
              outvalue.set(Double.toString(tweet.getGeoLocation().getLongitude()).getBytes());
              context.write(outkey, outvalue);
            }

            // (tweetId, "user", "id", user.id)
            outkey.set(new Key(tweetId, "user", "id"));
            outvalue.set(Long.toString(tweet.getUser().getId()).getBytes());
            context.write(outkey, outvalue);
