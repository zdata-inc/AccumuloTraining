...
public class TweetSearcher extends Configured implements Tool {
  ...
  public int run(String[] args) throws Exception {
    ...
    if (args.length < 1) {
      System.out.println(
          "Usage: accumulo-searcher-example.jar <space-delimited words to search for>");
      return 1;
    }
    ...
    BatchScanner batchScan =
        connector.createBatchScanner(Constants.TWEET_INDEX_TABLE, new Authorizations(), 10);
  ...
  public static void main(String[] args) throws Exception {
    System.exit(ToolRunner.run(new Configuration(), new TweetSearcher(), args));
  }
}
