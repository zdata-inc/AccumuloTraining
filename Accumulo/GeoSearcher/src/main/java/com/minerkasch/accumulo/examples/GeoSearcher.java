...
public class GeoSearcher extends Configured implements Tool {
  ...
  public int run(String[] args) throws Exception {
    ...
    if (args.length != 4) {
      System.err.println(
          "Usage: hadoop jar accumulo-geo-searcher.jar <lower_left_lat> "
    ...
    if (!connector.tableOperations().exists(Constants.TWITTER_TABLE)
        || !connector.tableOperations().exists(Constants.TWEET_LOCATION_INDEX_TABLE)) {
      System.out.format(
          "Error: Table %s or table %s does not exist",
          Constants.TWITTER_TABLE, Constants.TWEET_LOCATION_INDEX_TABLE);
      return 1;
    }

    // Create a batch scanner against the tweet location index table
    BatchScanner batchScan =
    ...
    for (Entry<Key, Value> record : batchScan) {
      ...
      double[] location = GeoUtil.decodeLocation(record.getKey().getRow().toString());
      ...
      if (location[0] >= llLat
        ...
        String tweetId = record.getKey().getColumnQualifier().toString();
    ...
    if (tweetIds.isEmpty()) {
      System.out.println("Found no tweets that matches the given bounding box");
      return 0;
    }

    // Initialize the batch scanner to scan the twitter data table with
    // the previously found tweet IDs as the ranges
    batchScan = connector.createBatchScanner(Constants.TWITTER_TABLE, new Authorizations(), 10);
  ...
  public static void main(String[] args) throws Exception {
    System.exit(ToolRunner.run(new Configuration(), new GeoSearcher(), args));
  }
}
